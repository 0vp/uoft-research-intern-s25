module rs_encode_wrapper(
    ...
    input scan_mode,
    input [8*27-1:0] datain,
    output reg [8*31-1:0] encoded_data,
    ...
    reg data_present;
    wire [7:0] encoded;
    reg [7:0] data_buffer[0:26]; // Buffer to hold input data
    integer i, j;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                S_LOAD: begin
                    enc_ena <= 1; // Enable encoder for 31 cycles
                    if (i < 27) begin
                        message <= data_buffer[i];
                        data_present <= 1; // Latch data for 27 cycles
                        i <= i + 1;
                        // if (i > 0) begin
