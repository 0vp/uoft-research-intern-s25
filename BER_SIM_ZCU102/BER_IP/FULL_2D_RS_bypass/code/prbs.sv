...
module prbs63_120_8 #(
    ...
    localparam WAIT_CYCLES = 500000;  // Simplified: N * 180 for typical case
    ...
    always @ (posedge clk) begin
        ...
        //end else if (stop) begin
            ...
            if (new_bit == 1) begin
                ...
                // new FEC symbol begins
                if (new_symbol==1) begin
                    new_symbol <= 0;
                    symbol_bit_index[cur_FEC_codeword] <= 1;
                    symbol_bit_errors[cur_FEC_codeword] <= error;
                    // codeword completion and error aggregation handled at symbol end
                end else begin
                ...
                // current FEC symbol complete: finalize symbol, then possibly finalize codeword
                if (symbol_bit_index[cur_FEC_codeword] == M-1) begin
                    // Include this symbol's error in codeword error count and handle frame boundary
                    if (symbol_codeword_index[cur_FEC_codeword] == N-1) begin
                        // Last symbol of the codeword just finished
                        if (symbol_codeword_errors[cur_FEC_codeword] + ((symbol_bit_errors[cur_FEC_codeword] + error) > 0) > T) begin
                            total_frame_errors <= total_frame_errors + 1;
                            // bit_codeword_errors already includes all bits of this codeword
                            total_bit_errors_post <= total_bit_errors_post + bit_codeword_errors[cur_FEC_codeword];
                        end
                        total_frames <= total_frames + 1;
                        // Reset per-codeword accumulators
                        symbol_codeword_index[cur_FEC_codeword] <= 0;
                        symbol_codeword_errors[cur_FEC_codeword] <= 0;
                        bit_codeword_errors[cur_FEC_codeword] <= 0;
                    end else begin
                        // Not the last symbol: roll forward counters
                        symbol_codeword_errors[cur_FEC_codeword] <= symbol_codeword_errors[cur_FEC_codeword] + ((symbol_bit_errors[cur_FEC_codeword] + error) > 0);
                        symbol_codeword_index[cur_FEC_codeword] <= symbol_codeword_index[cur_FEC_codeword] + 1;
                    end
                    // Advance to next symbol/codeword position
